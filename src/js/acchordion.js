...
function accordion(){
  ...
  for (let index = 0; index < accordionContainer.length; index++){
    ...
    addClass(button, "absolute block md:hidden right-0 transform -translite-y-1/2 focus:outline-none transition duration-200 rotata-0")
...
function onClickAccordion(){
  if(ulList.className.index("h-0") >-1 ){
    addClass(button, "rotate-180")
    addClass(ulList, "opacit-100")
